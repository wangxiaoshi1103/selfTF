...
# Lint as: python3
"""Custom formatting functions for Weibo dataset.
...
class WeiboFormatter(GenericDataFormatter):
  """Defines and formats data for the weibo dataset.
  ...
  _column_definition = [
      ...
      ('popular', DataTypes.REAL_VALUED, InputTypes.TARGET),  # target
      ...
      ('dayOfWeek', DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),  # 4：周期性时间特征
  ...
  def split_data(self, df, valid_boundary=1315, test_boundary=1339):
      ...
      df['minsFromFirst'] = (df['time'] - df['timeOnBoard'].min()) / np.timedelta64(1, 'm')
      df['minsFromFirst'] = df['minsFromFirst'].astype(float)
      ...
      df = df.groupby('id').filter(lambda x: (x['minsFromStart'].max() - x['minsFromStart'].min()) >= 60)
  ...
  def get_fixed_params(self):
    """Returns fixed model parameters for weibos."""
    ...
    weibo_params = load_yaml_params('conf/weibo.yaml')
    fixed_params = {
        'total_time_steps': weibo_params['total_time_steps'],   # wake up 从yaml配置中取
        'num_encoder_steps': weibo_params['num_encoder_steps'],
        'hidden_layer_size': weibo_params['hidden_layer_size'],
        'is_bert': weibo_params['is_bert'],
        'num_epochs': 1,
        'early_stopping_patience': 5,
        'multiprocessing_workers': 5
    }
