...
class GenericDataFormatter(abc.ABC):
  ...
  def get_fixed_params(self):
    ...
    Requires the following keys:
      'total_time_steps': Defines the total number of time steps used by SELFTF
  ...
  def get_column_definition(self):
    """"Returns formatted column definition in order expected by the SELFTF."""
    ...
    categorical_inputs = [
        tup for tup in column_definition if tup[1] in {DataTypes.CATEGORICAL, DataTypes.TEXT}  and
        tup[2] not in {InputTypes.ID, InputTypes.TIME}
    ]
  ...
  def _get_tft_input_indices(self):
    """Returns the relevant indexes and input sizes required by SELFTF."""
