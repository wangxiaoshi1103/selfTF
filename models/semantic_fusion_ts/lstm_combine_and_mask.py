...
class LSTMCombineAndMask(nn.Module):
    def __init__(self, input_size, num_inputs, hidden_layer_size, dropout_rate, use_time_distributed=False, batch_first=True, tower='add'):
        ...
        self.flattened_grn = GatedResidualNetwork(self.num_inputs*self.hidden_layer_size, self.hidden_layer_size, self.num_inputs, self.dropout_rate, use_time_distributed=use_time_distributed, return_gate=True, batch_first=batch_first, tower=tower)

        self.single_variable_grns = nn.ModuleList()
        for i in range(self.num_inputs):
            self.single_variable_grns.append(GatedResidualNetwork(self.hidden_layer_size, self.hidden_layer_size, None, self.dropout_rate, use_time_distributed=use_time_distributed, return_gate=False, batch_first=batch_first, tower=tower))
    ...
    def forward(self, embedding, additional_context=None):
        ...
        #pdb.set_trace()
        # expanded_static_context = torch.zeros_like(expanded_static_context)
        if additional_context is not None:
            ...
            sparse_weights, static_gate = self.flattened_grn(flattened_embedding)
